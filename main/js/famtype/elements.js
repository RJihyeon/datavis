...
d3.csv("./data/famtype/elements_alone.csv").then((data) => { // 초기 csv 파일 표시
    d3.select("button[data-src='./data/famtype/elements_alone.csv']").classed('active', true);
    ...
    hours(groupedData["한부모 연령별"]);// 초기 차트 표시
    ...
    d3.selectAll("#dataSelect input[type='button']")
        .on("click", function () {
            ...
            switch (group) {
                case "g1": hours(groupedData["한부모 연령별"]); avg(groupedData["한부모 연령별"]); compare(groupedData["한부모 연령별"]); break;
                case "g2": hours(groupedData["한부모 학력별"]); avg(groupedData["한부모 학력별"]); compare(groupedData["한부모 학력별"]); break;
                case "g3": hours(groupedData["혼인 상태별"]); avg(groupedData["혼인 상태별"]); compare(groupedData["혼인 상태별"]); break;
                case "g4": hours(groupedData["가구 구성별"]); avg(groupedData["가구 구성별"]); compare(groupedData["가구 구성별"]);  break;
                case "g5": hours(groupedData["가장 어린 자녀별"]); avg(groupedData["가장 어린 자녀별"]); compare(groupedData["가장 어린 자녀별"]); break;
                case "g6": hours(groupedData["종사상 지위별"]); avg(groupedData["종사상 지위별"]); compare(groupedData["종사상 지위별"]); break;
                case "g7": hours(groupedData["정부 지원 유형별"]); avg(groupedData["정부 지원 유형별"]); compare(groupedData["정부 지원 유형별"]); break;
                case "g8": hours(groupedData["소득 수준별"]); avg(groupedData["소득 수준별"]); compare(groupedData["소득 수준별"]); break;
                case "g9": hours(groupedData["한부모가된 기간별"]); avg(groupedData["한부모가된 기간별"]); compare(groupedData["한부모가된 기간별"]); break;
            }
        });
});
