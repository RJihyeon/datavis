// INITIALIZE
d3.csv(
  "./data/domestic_violence/가출_여부_및_이유_20240503234321_utf8.csv"
).then((data) => {
  // 초기 csv 파일 표시
  d3.select(
    "button[data-src='./data/domestic_violence/가출_여부_및_이유_20240503234321_utf8.csv']"
  ).classed("active", true);
  ...
  d3.select("#dataSelect input[type='button'][data-group='g1']").classed(
    "active",
    true
  ); // 초기 버튼 활성화

  d3.selectAll("#dataSelect input[type='button']").on("click", function () {
    ...
    switch (group) {
      case "g1":
        showStackedBarChart(groupedData["한부모 연령별"]);
        break;
      case "g2":
        showStackedBarChart(groupedData["한부모 학력별"]);
        break;
      case "g3":
        showStackedBarChart(groupedData["혼인 상태별"]);
        break;
      case "g4":
        showStackedBarChart(groupedData["가구 구성별"]);
        break;
      case "g5":
        showStackedBarChart(groupedData["가장 어린 자녀별"]);
        break;
      case "g6":
        showStackedBarChart(groupedData["종사상 지위별"]);
        break;
      case "g7":
        showStackedBarChart(groupedData["정부 지원 유형별"]);
        break;
      case "g8":
        showStackedBarChart(groupedData["소득 수준별"]);
        break;
      case "g9":
        showStackedBarChart(groupedData["한부모가된 기간별"]);
        break;
    }
  });
});
...
function showStackedBarChart(data) {
  ...
  const yScale = d3
    ...
    .domain([
      0,
      d3.max(data, (d) => +d["1~3시간"] + +d["4~6시간"] + +d["7시간 이상"]),
    ])
    .range([height, 0]);
  ...
  const yAxisGroup = svg.append("g").attr("class", "yAxis-style").call(yAxis);
  ...
  const stack = d3.stack().keys(["1~3시간", "4~6시간", "7시간 이상"]);
